...
// packages/core/dist/adapter/auto-detect.js
var MCP_INDICATORS = [
  "@modelcontextprotocol/sdk",
  "mcp",
  "fastmcp"
];
var hasMcpSignature = (input) => {
...
var detectAdapter = (input, adapters) => {
  ...
  const hasOpenApiSpec = input.files.some((file) => file.path === "openapi.json" || file.path === "openapi.yaml" || file.path === "openapi.yml");
  if (hasOpenApiSpec) {
...
// packages/core/dist/analyzers/env-extractor.js
var ENV_PATTERNS = [
  /process\.env\.([A-Z0-9_]+)/g,
  /os\.environ\[['"]([A-Z0-9_]+)['"]\]/g,
  /getenv\(['"]([A-Z0-9_]+)['"]\)/g
];
var extractEnvRefs = (bundle) => {
...
// packages/core/dist/analyzers/frontmatter.js
var import_yaml = __toESM(require_dist(), 1);

// packages/core/dist/config/defaults.js
var DEFAULT_GATE1_SCORE_THRESHOLD = 30;
var DEFAULT_GATE1_TIMEOUT_MS = 5e3;
var DEFAULT_RULE_SCAN_TIMEOUT_MS = 2e3;
var DEFAULT_MAX_ARCHIVE_BYTES = 10 * 1024 * 1024;
var DEFAULT_MAX_TOTAL_FILES = 200;
var DEFAULT_MAX_TOTAL_BYTES = 10 * 1024 * 1024;
var DEFAULT_MAX_SINGLE_FILE_BYTES = 1 * 1024 * 1024;
var DEFAULT_MAX_REFERENCES_FILES = 100;
var DEFAULT_MAX_REFERENCES_SINGLE_FILE_BYTES = 512 * 1024;
var DEFAULT_ALLOWED_TOOLS = ["Read", "Grep", "Glob"];
var ALLOWED_TOOLS_WHITELIST = [
  "Read",
  "Grep",
  "Glob",
  "Bash",
...
var parseFrontmatter = (content) => {
  ...
  const doc = (0, import_yaml.parseDocument)(yamlSource, { uniqueKeys: true });
  if (doc.errors.length > 0) {
...
var normalizeAllowedTools = (value) => {
  ...
  const tools = Array.isArray(value) ? value : typeof value === "string" ? value.split(",").map((part) => part.trim()).filter(Boolean) : [];
  if (tools.length === 0) {
    return [...DEFAULT_ALLOWED_TOOLS];
  }
  return tools.map((tool) => String(tool));
};
var validateAllowedTools = (tools) => {
  const allowList = new Set(ALLOWED_TOOLS_WHITELIST);
  return tools.filter((tool) => !allowList.has(tool));
};

// packages/core/dist/analyzers/structure.js
var toFinding = (id, message, file) => ({
  id,
  message,
  source: "layer0",
  file,
...
var isBinaryBuffer = (buffer) => {
  const sample = buffer.subarray(0, Math.min(buffer.length, 4096));
  return sample.includes(0);
};
var analyzeStructure = (bundle, options = {}) => {
  const findings = [];
  ...
  const maxReferencesSingleFileBytes = options.maxReferencesSingleFileBytes ?? DEFAULT_MAX_REFERENCES_SINGLE_FILE_BYTES;
  ...
  if (referencesFiles.length > maxReferencesFiles) {
    findings.push(toFinding("TOO_MANY_REFERENCE_FILES", `references directory exceeds ${maxReferencesFiles} files.`));
  }
  for (const file of bundle.files) {
    ...
    if (file.path.startsWith("references/")) {
      ...
      if (size > maxReferencesSingleFileBytes) {
        findings.push(toFinding("REFERENCES_FILE_TOO_LARGE", `references file exceeds ${maxReferencesSingleFileBytes} bytes.`, file.path));
      }
    }
  ...
  if (!manifest) {
    ...
    try {
      ...
      if (invalidTools.length > 0) {
        findings.push(toFinding("INVALID_ALLOWED_TOOLS", `allowed-tools contains unsupported entries: ${invalidTools.join(", ")}.`, "SKILL.md"));
      }
    } catch (error) {
  ...
  if (allowedTools.length === 0) {
    allowedTools = normalizeAllowedTools(void 0);
  }
  return {
...
// packages/core/dist/engine/rule-engine.js
import { extname } from "node:path";
var SNIPPET_CONTEXT = 2;
var SNIPPET_LINE_MAX = 200;
var buildSnippet = (lines, matchedLineIndex) => {
...
var isMentionContext = (path) => path.endsWith(".md");
var acceptsExtension = (rule, filePath) => {
...
var acceptsContext = (rule, filePath) => {
...
var evaluateRules = (bundle, rules) => {
  const findings = [];
  const hitFiles = /* @__PURE__ */ new Map();
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      for (const rule of rules) {
        ...
        if (!hitFiles.has(rule.id)) {
          hitFiles.set(rule.id, /* @__PURE__ */ new Set());
        }
...
// packages/core/dist/engine/rule-loader.js
import { readFile, readdir, stat } from "node:fs/promises";
var import_yaml2 = __toESM(require_dist(), 1);

// packages/core/dist/errors.js
var SkillGuardError = class extends Error {
  code;
...
var loadRulesFromFile = async (path) => {
...
// packages/core/dist/report/builder.js
var uniqueReasons = (reasons) => Array.from(new Set(reasons));
var buildReport = (input) => {
  const reasons = [];
...
var withTimeout = async (promise, timeoutMs, code) => {
...
var scanBundle = async (input) => {
  const { bundle, adapter, options: { scoreThreshold = DEFAULT_GATE1_SCORE_THRESHOLD, gate1TimeoutMs = DEFAULT_GATE1_TIMEOUT_MS, ruleScanTimeoutMs = DEFAULT_RULE_SCAN_TIMEOUT_MS, rulesDir, reportId, now } } = input;
  const run = async () => {
    ...
    const rules = await withTimeout(loadRules(rulePaths), ruleScanTimeoutMs, "GATE1_TIMEOUT").catch((error) => {
    ...
    const dependencies = uniqueBy([...adapterDependencies, ...extractDependencies(bundle)], (dep) => `${dep.file}:${dep.source}:${dep.name}`);
    const envRefs = uniqueBy([...adapterEnvRefs, ...extractEnvRefs(bundle)], (envRef) => `${envRef.file}:${envRef.name}`);
    return buildReport({
      reportId,
      createdAt: now,
      scoreThreshold,
...
// packages/core/dist/report/sarif.js
var severityToLevel = (severity) => {
  switch (severity) {
    case "Critical":
      return "error";
    case "High":
...
var toSarif = (report, version = "0.1.0") => {
...
var parseSimpleFrontmatter = (content) => {
...
var NewmaxAdapter = class {
  id = "newmax";
  async parseBundle(input) {
  ...
  async extractMetadata(bundle) {
    ...
    const allowedTools = Array.isArray(allowedToolsRaw) ? allowedToolsRaw.map((item) => String(item)) : void 0;
    return {
  ...
  async extractDependencies(bundle) {
    return extractDependencies(bundle);
  }
  async extractEnvRefs(bundle) {
...
var line = (width) => "\u2500".repeat(width);
var formatTerminal = (report) => {
  const width = 55;
  const lines = [];
...
var formatJson = (report) => {
  return JSON.stringify(report, null, 2);
};

// packages/cli/src/output/sarif.ts
var formatSarif = (report) => {
...
var findRulesDir = () => {
  ...
  for (let i = 0; i < 8; i += 1) {
    const candidate = resolve2(here, "rules/base/common.yaml");
    if (existsSync(candidate)) return resolve2(here, "rules/base");
    const parent = dirname(here);
    if (parent === here) break;
    here = parent;
  }
  return resolve2(import.meta.dirname, "../../../../rules/base");
};
var DEFAULT_RULES_DIR = findRulesDir();
var parseOptions = (args) => {
  ...
  return {
    path: resolve2(positionals[0]),
    options: {
      mode: values.mode ?? "standard",
      format: values.format ?? "terminal",
      adapter: values.adapter ?? "auto",
      rulesDir: values["rules-dir"] ? resolve2(values["rules-dir"]) : DEFAULT_RULES_DIR,
      threshold: parseInt(values.threshold, 10) || 30
    }
  };
};
var readSkillDirectory = async (dirPath) => {
  const files = [];
...
var scanCommand = async (args) => {
  ...
  const adapters = [new NewmaxAdapter()];
  const input = {
...
var findRulesDir2 = () => {
  let here = import.meta.dirname;
  for (let i = 0; i < 8; i += 1) {
    if (existsSync2(resolve3(here, "rules/base/common.yaml"))) return resolve3(here, "rules/base");
    const parent = dirname2(here);
    if (parent === here) break;
    here = parent;
  }
  return resolve3(import.meta.dirname, "../../../../rules/base");
};
var DEFAULT_RULES_DIR2 = findRulesDir2();
var readSkillDirectory2 = async (dirPath) => {
  const files = [];
  const walk = async (dir, prefix) => {
...
var reportCommand = async (args) => {
  if (args.length === 0) {
    throw new Error("Please provide a path. Usage: skill-guard report <path>");
  }
  const targetPath = resolve3(args[0]);
  const targetStat = await stat3(targetPath);
  if (!targetStat.isDirectory()) {
    throw new Error(`Expected a directory path, got: ${targetPath}`);
  }
  const rawFiles = await readSkillDirectory2(targetPath);
  const adapters = [new NewmaxAdapter()];
  const input = {
...
var updateRulesCommand = async (_args) => {
...
// packages/cli/src/bin/skill-guard.ts
var VERSION = "0.1.0";
var showHelp = () => {
  console.log(`
